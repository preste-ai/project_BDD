...
def train_on_device(obj_names, args, use_cuda = True):
    ...
    for obj_name in obj_names:
        run_name = 'DRAEM_train_'+str(args.lr)+'_'+str(args.epochs)+'_bs'+str(args.bs)+"_"+obj_name+'_'

        visualizer = TensorboardVisualizer(log_dir=os.path.join(args.log_path, run_name+"/"))

        model = ReconstructiveSubNetwork(in_channels=3, out_channels=3, base_width=32)
        if use_cuda:
            model.cuda()
        model.apply(weights_init)

        model_seg = DiscriminativeSubNetwork(in_channels=6, out_channels=2, base_channels=32)
        if use_cuda:
            model_seg.cuda()
        ...
        dataset = MVTecDRAEMTrainDataset(args.data_path + obj_name + "/", args.anomaly_source_path, resize_shape=[256, 256])

        dataloader = DataLoader(dataset, batch_size=args.bs,
                                shuffle=True, num_workers=4)
        print('N images in training set: {}'.format(len(dataloader)))
        n_iter = 0
        #print('F : torch.cuda.memory_reserved(device=torch.cuda.device(0)) - > ', torch.cuda.memory_reserved(device=torch.cuda.device(0)))
        for epoch in range(args.epochs):
            ...
            for i_batch, sample_batched in enumerate(dataloader):
                if use_cuda:
                    ...
                    aug_gray_batch = sample_batched["augmented_image"]
...
if __name__=="__main__":
    ...
    if int(args.gpu_id) == -1:
        train_on_device(picked_classes, args, use_cuda=False)
